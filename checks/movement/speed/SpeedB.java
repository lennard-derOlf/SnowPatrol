...
public class SpeedB extends Check {
	...
	public static boolean isReallyOnGround(Player p) {
        ...
        return p.isOnGround() && b.getBlock().getType() != Material.AIR && b.getBlock().getType() != Material.COBWEB
                && !b.getBlock().isLiquid();
    }
    public static boolean flaggyStuffNear(Location loc) {
        boolean nearBlocks = false;
        for (Block bl : UtilBlock.getSurrounding(loc.getBlock(), true)) {
            if ((bl.getType().equals(Material.STONE_SLAB)) || (bl.getType().equals(Material.SMOOTH_STONE))
                    || (bl.getType().equals(Material.RED_BED)) || (bl.getType().equals(Material.OAK_PLANKS))
        ...
        if (isBlock(loc.getBlock().getRelative(BlockFace.DOWN), new Material[]{Material.STONE_SLAB, Material.RED_BED,
                Material.SMOOTH_STONE, Material.OAK_PLANKS, Material.OAK_SLAB})) {
            nearBlocks = true;
        }
        return nearBlocks;
    }
    ...
    public void onMove(PlayerMoveEvent e) {
        ...
        if (hadSpeed.contains(p.getUniqueId())) {
            maxSpeed += .5;
            Bukkit.getScheduler().runTaskLaterAsynchronously(ThotPatrol.Instance, () -> {
        ...
        if (p.getEyeLocation().clone().add(0, 1, 0).getBlock().getType().isSolid()
                ...
                || p.getLocation().add(0, 1, 0).getBlock().getType().equals(Material.OAK_TRAPDOOR)
        ...
        if (below.equals(Material.ICE) || below.equals(Material.PACKED_ICE) 
        	...
        	Bukkit.getScheduler().runTaskLaterAsynchronously(ThotPatrol.Instance, new Runnable() {
        		@Override
        		public void run() {
        ...
        if (isReallyOnGround(p) && to.getY() == from.getY()) {
            if (speed >= maxSpeed && p.isOnGround() && p.getFallDistance() < 0.15
                    ...
                    && loc2.getBlock().getType() != Material.OAK_TRAPDOOR && above.getBlock().getType() == Material.AIR
